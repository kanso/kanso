...
exports.availableVersions = function (candidates, callback) {
    async.map(candidates, function (c, cb) {
        path.exists(path.join(c, 'kanso.json'), function (exists) {
            if (exists) {
                return settings.load(c, function (err, doc) {
                    cb(err, {path: c, doc: doc});
                });
            }
            cb();
        });
    ...
    function (err, results) {
        ...
        callback(null, versions.reduceVersions(results, versions));
    });
};
...
exports.resolve = async.memoize(function (name, ranges, paths, source, callback) {
    ...
    exports.availableVersions(candidates, function (err, matches) {
        ...
        if (highest) {
            ...
            return callback(null, highest, m.doc, m.path);
        }
        if (versions.length) {
            var e = new Error(
