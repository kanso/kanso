    ...
    exports.resolve(name, paths, source, function (err, p) {
        ...
        settings.load(p, function (err, cfg) {
            ...
            async.reduce(deps, {}, function (doc, k, cb) {
                exports.load(plugins, k, false, paths, p,
                    function (err, nd, nc, np, already_loaded) {
                        ...
                        var post = plugins_module.postprocessors(plugins);
                        exports.process(pre, post, false, np, nc, nd,
                            function (err, nd, nc) {
                                if (err) {
                                    return cb(err);
                                }
            ...
            function (err, doc) {
                ...
                var pre  = plugins_module.preprocessors(plugins);
                var post = plugins_module.postprocessors(plugins);
                exports.process(pre, post, root, p, cfg, doc, callback);
            });
        });
    });
};
...
exports.process = function (pre, post, root, p, cfg, doc, callback) {
    async.reduce(pre, doc, function (doc, fn, cb) {
        fn(root, p, cfg, doc, cb);
    },
    function (err, doc) {
        exports._loaded_packages[cfg.name] = p;
        if (root) {
            console.log('running post processors on ' + p);
            // run post-processors on merged document
            async.reduce(post, doc, function (doc, fn, cb) {
                fn(p, cfg, doc, cb);
            },
            function (err, doc) {
