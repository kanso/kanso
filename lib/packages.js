                ...
                function (err, doc, pre, post) {
                    ...
                    exports.process(pre, post, root, p, cfg, doc,
                        function (err, doc, cfg, p, already_loaded, post) {
                            ...
                            if (root) {
                                // add values from kanso.json if this is the
                                // root package
                                var now = new Date();
                                cfg._build_time = utils.ISODateString(now);
                                doc.kanso = cfg;
                            }
...
exports.loadDependencies = function (p, cfg, paths, source, options, callback) {
    ...
    async.forEach(deps, function (k, cb) {
        var range = cfg.dependencies[k];
        exports.resolve(k, range, paths, source, function (err, np) {
            if (np === p) {
                return callback(new Error(
                    'Package should not specify itself as a ' +
                    'dependency: ' + p
                ));
            }
            exports.load(k, false, paths, p, options,
                function (err, nd, nc, np, already_loaded, _post) {
                    if (err) {
                        return cb(err);
                    }
                    // add postprocessors from dependencies
                    _.extend(post, _post);
                    if (nc.preprocessors) {
...
exports.process = function (pre, post, root, p, cfg, doc, callback) {
    exports.preprocess(pre, root, p, cfg, doc, function (err, doc) {
        ...
        if (root) {
            ...
            callback(err, utils.stringifyFunctions(doc), cfg, p, false, post);
        }
    });
};
...
exports.resolve = async.memoize(function (name, range, paths, source, callback) {
    ...
    async.map(candidates, function (c, cb) {
        path.exists(path.join(c, 'kanso.json'), function (exists) {
            if (exists) {
                return settings.load(c, function (err, cfg) {
                    cb(err, {path: c, cfg: cfg});
                });
            }
            cb();
        });
    ...
    function (err, results) {
        ...
        results.forEach(function (r) {
            if (r) {
                ...
                if (!matches[r.cfg.version]) {
                    matches[r.cfg.version] = r.path;
                }
            }
        });
