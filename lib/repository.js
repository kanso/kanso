...
exports.availableVersions = function (name, repositories, callback) {
    async.map(repositories, function (repo, cb) {
        exports.get(repo, name, true, function (err, doc) {
            return cb(err, {repository: repo, doc: doc});
        });
    },
    function (err, results) {
        if (err) {
        ...
        // results are in original order of repositories at this stage
        var versions = results.reduce(function (versions, r) {
            if (!r.doc || !r.doc.versions) {
                return versions;
            }
            for (var v in r.doc.versions) {
                // don't overwrite exising versions from higher-priority repos
                if (!versions[v]) {
                    versions[v] = r;
                }
            }
...
exports.maxSatisfying = function (name, ranges, repositories, callback) {
    ...
    exports.availableVersions(name, repositories, function (err, vers) {
        ...
        var satisfying = Object.keys(vers).filter(function (v) {
            return ranges.every(function(r) {
                // if range is null, everything satisfies it
                return !r || semver.satisfies(v, r);
            });
        });
        // semver.rcompare not working here for some reason
        var max = satisfying.sort(semver.compare).reverse()[0];
        if (!max) {
            return callback(null, null);
        }
