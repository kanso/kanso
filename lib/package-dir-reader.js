...
/**
 * Override the IgnoreReader's emitEntry for our own purposes.
 * Ignore symbolic links, emit files after prefixing with "package" and
 * don't emit folders but do wire them up for emitting containing files.
 */
...
/**
 * Prefix the entry's path with a "package" folder
 */
function updateEntryPath(entry) {
  var relativePath = path.relative(entry.root.path, entry.path);
  var newPath = path.resolve(entry.root.path, 'package', relativePath);
  entry.path = newPath;
}
