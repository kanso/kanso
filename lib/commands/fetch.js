...
function satisfiesAll(version, ranges) {
    console.log(['satisfiesAll', version, ranges]);
...
function install(name, range, data, parent, callback) {
            ...
            if (OPTIONS.force && !removed[name]) {
                ...
                utils.rm('-rf', target_dir + '/' + name, function (err) {
                    ...
                    process.nextTick(function () {
                        install(name, range, data, parent, callback);
                    });
                });
            }
            else {
...
function worker(task, callback) {
    repository.resolve(task.name, task.range, repo, function (err, v, data) {
        ...
        install(task.name, v, data, task.parent, function (err, version, cfg) {
            if (err) {
                return logger.error(err);
            }
            fetched.push({
