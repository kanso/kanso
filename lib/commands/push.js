...
exports.run = function (settings, args) {
    ...
    path.exists(dir, function (exists) {
        ...
        kansorc.extend(settings, dir + '/.kansorc', function (err, settings) {
            ...
            url = utils.argToURL(settings, url);
...
exports.loadApp = function (dir, url, options, settings, callback) {
    ...
    packages.load(dir, true, paths, null, options, function (err, doc, cfg) {
        ...
        utils.catchAuthError(exports.push, url, [dir, cfg, doc], callback);
    });
};


/**
 * Make sure the DB exists, if it doesn't create it
 */

exports.ensureDB = function (url, callback) {
    couchdb(url).ensureDB(callback);
};
...
exports.push = function (url, dir, cfg, doc, callback) {
    utils.completeAuth(url, false, function (err, url) {
        if (err) {
            return callback(err);
        }
        var db = couchdb(url);
        utils.catchAuthError(exports.ensureDB, url, [], function (err) {
            if (err) {
                return callback(err);
            }
