...
CouchDB.prototype.client = function (method, path, data, callback) {
	...
	kansorc.load(function(err, settings) {
	    var pathname = that.instance.pathname;
	    ...
	    if (that.instance.auth) {
	        var enc = new Buffer(that.instance.auth).toString('base64');
	        headers.Authorization = "Basic " + enc;
	    }

	    var proto = (that.instance.protocol === 'https:') ? https: http;

	    var req_opts = {};
	    if (settings.proxy.host && settings.proxy.port) {
	    	req_opts = {
	        	host: settings.proxy.host, //that.instance.hostname,
	        	port: settings.proxy.port, //that.instance.port,
	        	method: method,
	        	path: that.instance.protocol + '//' + that.instance.hostname + ':' + that.instance.port + path,
	        	headers: headers
	    	};	    	
	    } else {
	    	req_opts = {
	    ...
	    var request = proto.request(req_opts);
